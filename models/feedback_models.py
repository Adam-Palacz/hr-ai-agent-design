...
class HRFeedback(BaseModel):
    ...
    weaknesses: List[str] = Field(
        default_factory=list, description="Areas for improvement or concerns"
    )
    ...
    interviewer_name: Optional[str] = Field(
        None, description="Name of the interviewer or HR representative"
    )
    missing_requirements: List[str] = Field(
        default_factory=list, description="Missing required qualifications"
    )
...
class CandidateFeedback(BaseModel):
    ...
    html_content: str = Field(
        ...,
        description="Complete HTML formatted feedback email ready to send. This is the ONLY required field. The HTML should include greeting, decision announcement, strengths, improvement areas, next steps, and closing - all formatted as a complete HTML email with inline styles.",
    )

...
def get_ai_footer() -> str:
    """Get AI footer with privacy policy link if available."""
...
AI_FOOTER = get_ai_footer()  # For backward compatibility, but will be regenerated each time
...
CONSENT_MESSAGE_NO = """<p style="margin-top: 20px; padding: 15px; background-color: #f9f9f9; border-left: 4px solid #667eea; border-radius: 4px;">
    ...
    Zauważyliśmy, że nie wyraziłeś zgody na rozważanie Twojej kandydatury w kontekście innych stanowisk. Jeżeli chciałbyś to zmienić, prosimy o odpowiedź na tę wiadomość z informacją, że wyrażasz zgodę.
</p>"""
...
CONSENT_MESSAGE_YES = """<p style="margin-top: 20px; padding: 15px; background-color: #f0f9ff; border-left: 4px solid #4caf50; border-radius: 4px;">
    <strong>Informacja dotycząca innych stanowisk:</strong><br>
    Zauważyliśmy, że wyraziłeś zgodę na uwzględnienie Twojej kandydatury przy rekrutacji na inne stanowiska. Dlatego gdy znajdziemy odpowiednią dla Ciebie ofertę, niezwłocznie się odezwiemy. Jeżeli natomiast chciałbyś zmienić zdanie, prosimy o poinformowanie nas o tym w odpowiedzi na tę wiadomość.
</p>"""


def format_feedback_as_html(
    feedback: CandidateFeedback, consent_for_other_positions: bool = False
) -> str:
    """

...
def format_feedback_as_html(feedback: CandidateFeedback, consent_for_other_positions: bool = None) -> str:
    """
    ...
    if hasattr(feedback, 'html_content') and feedback.html_content:
        ...
        # Add hardcoded consent message before footer (if consent info is available)
        ...
        elif consent_for_other_positions is False:
            consent_message = CONSENT_MESSAGE_NO
        # If consent_for_other_positions is None, no message is added
        
        # Add hardcoded footer and consent message before closing </body> tag
        ...
        if "Informacja ta została wygenerowana automatycznie" not in html_content:
            # Insert hardcoded consent message (if any) and hardcoded footer before </body> tag
            html_content = re.sub(
                r'(</body>)',
                consent_message + AI_FOOTER + r'\1',
                html_content,
                flags=re.IGNORECASE
            )

...
# Check for pdf2image (import is the availability test; noqa for "unused" import)
try:
    from pdf2image import convert_from_path, convert_from_bytes  # noqa: F401

    PDF2IMAGE_AVAILABLE = True
except ImportError:
    PDF2IMAGE_AVAILABLE = False

# Check for PyMuPDF (import is the availability test; noqa for "unused" import)
try:
...
def extract_text_from_pdf_with_ocr(
    pdf_path: str, vision_model, use_ocr: bool = True, verbose: bool = False
) -> str:
    """
    ...
    try:
        ...
        if verbose:
            print(
                f"  ✅ Converted {len(image_bytes_list)} pages to images ({image_conversion_time:.2f}s)"
            )
    except Exception as e:
    ...
    try:
        ...
        for page_num, image_bytes in enumerate(image_bytes_list):
            ...
            logger.debug(
                f"Page {page_num + 1}: Image size {image_size_kb:.2f} KB, base64 length: {len(base64_image)}"
            )
            ...
            if verbose:
                print("    ⏳ Sending to vision model (this may take 10-30 seconds per page)...")
            ...
            logger.info(
                f"Page {page_num + 1} OCR completed: {len(page_text)} characters in {page_processing_time:.2f}s (vision: {vision_request_time:.2f}s)"
            )
            ...
            if page_text.strip():
                ...
                if verbose:
                    print(
                        f"    ✅ Extracted {len(page_text)} characters from page {page_num + 1} ({page_processing_time:.2f}s)"
                    )
        ...
        logger.error(
            f"Error extracting text with vision model: {type(e).__name__}: {str(e)}", exc_info=True
        )
        raise Exception(f"Error extracting text with vision model: {str(e)}")


def extract_text_from_pdf(
    pdf_path: str, vision_model=None, use_ocr: bool = True, verbose: bool = False
) -> str:
    """
    ...
    try:
        ...
        with open(pdf_path, "rb") as file:
            ...
            logger.info(
                f"Text extraction completed: {len(extracted_text)} characters in {text_extraction_time:.2f}s"
            )
            if verbose:
                print(
                    f"  ✅ Extracted {len(extracted_text)} characters of text ({text_extraction_time:.2f}s)"
                )
            ...
            if len(extracted_text.strip()) < 100 and vision_model and use_ocr:
                logger.warning(
                    f"Too little text extracted ({len(extracted_text.strip())} chars), switching to OCR"
                )
                if verbose:

...
- Continue with: "Chciałbym/chciałabym podzielić się z Tobą opinią dotyczącą Twojej kandydatury."
- Then smoothly integrate the decision - connect it naturally with the opening paragraph
- For rejections, flow naturally: "...Chciałbym podzielić się z Tobą opinią dotyczącą Twojej kandydatury. Z przykrością informujemy, że zdecydowaliśmy się procedować z innymi kandydatami."
...
- Extract and highlight strengths from HR notes AND from the candidate's CV that match the job requirements - be genuine and specific
- Extract and provide constructive feedback on areas for improvement from HR notes AND identify any gaps when comparing CV to job requirements - be encouraging, not critical
...
- Be specific and actionable in your feedback, but frame it as helpful suggestions
...
1. A warm, natural greeting that smoothly transitions into the decision announcement
   ...
   - Continue with: "Chciałbym/chciałabym podzielić się z Tobą opinią dotyczącą Twojej kandydatury."
   - Then naturally flow into the decision
   - EXAMPLE STRUCTURE:
     "Dziękujemy za złożenie aplikacji na stanowisko Senior DevOps Engineer w TechCorp Inc.. Chciałbym podzielić się z Tobą opinią dotyczącą Twojej kandydatury. [Then decision]"
2. Clear but gentle decision announcement (accepted/rejected) - be honest but compassionate
   - CRITICAL FOR REJECTIONS: When the decision is "rejected", you MUST use the following phrase: 
   ...
   - This phrase should be NATURALLY INTEGRATED into the opening paragraph, not as a separate section
   - Connect it smoothly with the greeting: "Dziękujemy za złożenie aplikacji... Chciałbym podzielić się z Tobą opinią... Z przykrością informujemy, że zdecydowaliśmy się procedować z innymi kandydatami."
   - Make it flow naturally - the decision should feel like a continuation of the introduction, not a separate announcement
...
3. Detailed strengths section (extract from HR notes and CV) - reference specific experiences, skills, achievements
4. Constructive improvement areas (extract from HR notes and identify gaps) - be encouraging, not critical
5. Next steps or recommendations (if accepted: next steps; if rejected: how to improve with encouragement)
6. Warm, friendly closing - end on a positive, supportive note
...
HTML formatted version (html_content field) - THIS IS THE ONLY REQUIRED FIELD:
   ...
   - Identify and bold key terms relevant to ANY industry (IT, finance, marketing, sales, HR, operations, healthcare, etc.)
...
- MANDATORY BOLDING (always bold these):
  ...
  * Job position/title: Always bold the complete job title/position name (e.g., "<strong>Senior DevOps Engineer</strong>", "<strong>Marketing Manager</strong>")
- Examples of CORRECT bolding:
  ...
  * "<strong>Infrastructure as Code</strong>" (correct) vs "<strong>Infrastructure</strong> as Code" (wrong)
  ...
  * "application for the <strong>Senior DevOps Engineer</strong> position at <strong>TechCorp Inc.</strong> - <strong>Z przykrością informujemy, że zdecydowaliśmy się procedować z innymi kandydatami</strong>" (correct)

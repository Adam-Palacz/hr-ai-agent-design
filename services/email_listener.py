...
class EmailListener:
    ...
    IOD_KEYWORDS = [
        ...
        'sprzeciw', 'zgoda', 'wycofanie', 'skarga', 'uodo',
        'organ nadzorczy', 'profilowanie', 'automatyczna decyzja',
        'sztuczna inteligencja', 'ai', 'si'
    ]
    
    # Keywords for consent classification
    CONSENT_KEYWORDS_POSITIVE = [
        'zgoda', 'zgadzam się', 'wyrażam zgodę', 'tak', 'chcę',
        'zainteresowany', 'rozważenie', 'inne oferty', 'inne stanowiska',
        'inne pozycje', 'inne rekrutacje'
    ]
    
    CONSENT_KEYWORDS_NEGATIVE = [
        'nie zgadzam się', 'odmawiam', 'nie', 'nie chcę', 'nie wyrażam zgody',
        'wycofuję zgodę', 'nie jestem zainteresowany', 'nie rozważaj'
    ]
    ...
    def get_unread_emails(self, folder: str = 'INBOX') -> List[Dict]:
        ...
        try:
            ...
            if status != 'OK':
                logger.error(f"Failed to select folder {folder} (status={status}, messages={messages})")
                return []
            
            # Search for unread emails
            status, message_numbers = self.mail.search(None, 'UNSEEN')
            if status != 'OK':
                logger.error(f"Failed to search for unread emails (status={status}, data={message_numbers})")
                return []
    ...
    def classify_email(self, email_data: Dict, classifier_agent=None) -> str:
        ...
        if classifier_agent:
            try:
                ...
                logger.warning(f"AI classification failed, falling back to keyword matching: {str(e)}")

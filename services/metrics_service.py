...
from database.models import (
    get_db, get_all_candidates, get_all_positions, get_all_feedback_emails, 
    get_all_model_responses, get_all_validation_errors, get_all_tickets, get_all_hr_notes
)
...
class MetricsService:
    ...
    def get_feedback_metrics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_feedback = [
                fe for fe in feedback_emails
                if fe.sent_at and fe.sent_at >= cutoff_date
            ]
            ...
            recent_errors = [
                ...
                if ve.created_at and ve.created_at >= cutoff_date
            ]
            
            # Get model responses for feedback generation
            model_responses = get_all_model_responses()
            feedback_responses = [
                mr for mr in model_responses
                if mr.agent_type == "feedback_generator"
                and mr.created_at and mr.created_at >= cutoff_date
            ]
            
            validation_responses = [
                mr for mr in model_responses
                if mr.agent_type == "validator"
                and mr.created_at and mr.created_at >= cutoff_date
            ]
            ...
            for mr in validation_responses:
                ...
                if metadata:
                    try:
                        ...
                        validation_iterations[iteration] = validation_iterations.get(iteration, 0) + 1
                    except:
                        pass
            ...
            return {
                ...
                "error_rate": round(total_errors / total_feedback * 100, 2) if total_feedback > 0 else 0.0,
    ...
    def get_cost_metrics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_responses = [
                mr for mr in model_responses
                if mr.created_at and mr.created_at >= cutoff_date
            ]
            ...
            return {
                ...
                "cost_by_agent": {
                    agent: {
                        ...
                        "count": stats["count"],
                        "avg_cost_pln": round(stats["cost"] / stats["count"], 4) if stats["count"] > 0 else 0.0,
    ...
    def get_email_processing_metrics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            classifier_count = len([
                mr for mr in email_responses
                if mr.agent_type == "email_classifier"
            ])
            
            query_classifier_count = len([
                mr for mr in email_responses
                if mr.agent_type == "query_classifier"
            ])
            ...
            return {
                ...
                "emails_per_day": round((classifier_count + query_classifier_count) / days, 2) if days > 0 else 0
            }
        except Exception as e:

...
class EmailRouter:
    ...
    def __init__(
        ...
        try:
            ...
            logger.warning(f"Failed to initialize AI client: {e}. Will use default priority/deadline.")
            self.ai_client = None
        ...
        try:
            ...
            logger.warning(f"Failed to initialize query agents: {e}. Will forward all queries to HR.")
    ...
    def route_email(self, email_data: Dict, classification: str) -> bool:
        ...
        try:
            ...
            if email_id:
                ...
                logger.debug(f"Marking email {email_id} as processed (classification: {classification})")
                ...
                if len(self.processed_emails) > self.max_processed_emails:
                    ...
                    logger.debug(f"Trimmed processed_emails set to {len(self.processed_emails)} entries")
    ...
    def _route_to_iod(self, email_data: Dict) -> bool:
        ...
        try:
            ...
            if message_id:
                try:
                    ...
                    existing_ticket = next(
                        (t for t in all_tickets if t.related_email_id == message_id),
                        None
                    )
                    if existing_ticket:
                        logger.info(f"Ticket #{existing_ticket.id} already exists for email {message_id}, skipping duplicate ticket creation")
                except Exception as e:
            ...
            if existing_ticket:
                logger.info(f"Email {message_id or 'with UID'} already processed (ticket #{existing_ticket.id} exists), skipping duplicate processing")
                return True  # Return True to avoid reprocessing
    ...
    def _handle_general_query(self, email_data: Dict) -> bool:
        ...
        try:
            ...
            if action == 'forward_to_hr':
                ...
                reply_subject = f"Re: {email_subject}" if email_subject else "Odpowiedź na Twoje zapytanie"
                success = self._send_email(
                    to_email=from_email,
                ...
                if self.rag_validator:
                    ...
                    if not validation_result.is_approved:
                        ...
                        logger.info(f"RAG response validation PASSED for {from_email}: {validation_result.reasoning}")
                else:
                    logger.warning("RAG validator not available, skipping validation (sending response anyway)")
                ...
                if success:
                    ...
                    # Also notify HR about the auto-response
                    self._notify_hr_about_auto_response(email_data, response, "rag_answer", rag_results)
...
"""
    ...
    def _determine_ticket_priority_and_deadline(self, email_subject: str, email_body: str, from_email: str) -> tuple:
        """
...
"""
            ...
            response = self.ai_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You are an expert in prioritizing HR inquiries and determining appropriate response deadlines."},
            ...
            logger.info(f"AI determined priority: {priority_str}, deadline: {deadline_days} days. Reasoning: {reasoning}")
            
            return priority, deadline_days
            
        except Exception as e:
            logger.warning(f"Error determining ticket priority/deadline with AI: {str(e)}. Using defaults.")
            from database.models import TicketPriority
    ...
    def _route_to_hr(self, email_data: Dict) -> bool:
        ...
        try:
            ...
            from database.models import (
                create_ticket, TicketDepartment, TicketPriority, TicketStatus,
                get_candidate_by_email, get_position_by_id, get_all_tickets, get_all_candidates
            )
            ...
            try:
                ...
                if candidates_with_email:
                    ...
                    logger.info(f"Found candidate {related_candidate_id} ({latest_candidate.full_name}) for email {from_email}")
            except Exception as e:
                logger.warning(f"Error finding candidate for email {from_email}: {str(e)}")
            ...
            if not existing_ticket:
                ...
                try:
                    ...
                    logger.info(f"Created ticket #{ticket.id} for HR inquiry from {from_email} (priority: {priority.value}, deadline: {deadline_days} days)")
                except Exception as e:
            ...
            if candidate:
                try:
                    ...
                    stage_display = {
                        ...
                        'offer': 'Oferta'
                    }.get(candidate.stage.value if hasattr(candidate.stage, 'value') else str(candidate.stage), str(candidate.stage))
                    
                    status_display = {
                        'in_progress': 'W trakcie',
                        'accepted': 'Zaakceptowany',
                        'rejected': 'Odrzucony'
                    }.get(candidate.status.value if hasattr(candidate.status, 'value') else str(candidate.status), str(candidate.status))
...
---
"""
                    logger.info(f"Found candidate {candidate.id} ({candidate.full_name}) for email {from_email}")
                except Exception as e:
    ...
    def _send_email(self, to_email: str, subject: str, body: str, reply_to: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if not body or not body.strip():
                logger.warning(f"Attempted to send empty email to {to_email} with subject: {subject}. Skipping.")
                return False
            
            if not subject or not subject.strip():
                logger.warning(f"Attempted to send email to {to_email} with empty subject. Skipping.")
                return False
